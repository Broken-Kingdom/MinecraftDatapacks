...
### Life giver/taker ###
########################
execute as @a[gamemode=!spectator,tag=!Dimmune] at @s if score @s deaths = @e[type=armor_stand,name=LoadStatus,limit=1] maxDeaths run gamemode spectator @s
execute as @a[gamemode=!survival,tag=!Dimmune] at @s unless score @s deaths >= @e[type=armor_stand,name=LoadStatus,limit=1] maxDeaths run gamemode survival @s
